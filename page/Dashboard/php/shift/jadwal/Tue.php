...
<?php
     ...
     $shift=sprintf("SELECT * FROM shift WHERE id_usaha=(SELECT id_usaha FROM usaha WHERE email='%s') AND hari='Tue'",$_SESSION["EPemilik"]);
     //echo $shift;
     ...
     if(mysqli_num_rows($queryShift)>0){
          while($jadwalShift = mysqli_fetch_array($queryShift)){
               ...
               $syntax= sprintf("SELECT * FROM pegawai WHERE  day2='%s'",$jadwalShift["id_shift"]);
